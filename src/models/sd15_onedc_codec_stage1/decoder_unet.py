...
from diffusers.models.unets.unet_2d_blocks import Transformer2DModel
...
def forward_unet(
    ...
    return sample, reduced_sample


def unet_set_grad(self, is_lora=False, is_all_model=False):
    # 1. set all parameters to be not requires_grad
    for param in self.parameters():
        param.requires_grad = False
            
    # 2. set the parameters of the conv_in to be requires_grad
    self.conv_in.requires_grad_(True)
    self.vae_reduction.requires_grad_(True)
            
    # 3. set the lora layers to be requires_grad
    if is_lora:
        for name, param in self.named_parameters():
            if "lora" in name:
                param.requires_grad = True
        print("[INFO] using lora to Unet")
    
    # 4. set all model to be requires_grad
    if is_all_model:
        for param in self.parameters():
            param.requires_grad = True
        print("[INFO] tunning all unet")
...
def prepare_unet_for_codec(in_ch: int, pretrained_path: str=None, lora_config=None):
    ...
    if pretrained_path is not None:
        print(feedforward_model.load_state_dict(torch.load(pretrained_path, map_location="cpu"), strict=True))
    ...
    # 4. add vae_reduction
    vae_reduction = reduce_resblock(in_ch, vae_ch)
    ...
    # 6. add set_grad function
    feedforward_model.manually_set_grad = unet_set_grad.__get__(feedforward_model)
